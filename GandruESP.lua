...
if not Degrade then
	...
	warn("Gandru_ESP > Jūsų išnaudojimas nepalaiko šio modulio optimizavimo! Vaizdiniai elementai gali vėluoti ir sumažinti našumą.")
end
...
if GandruESP and GandruESP.Exit then
	GandruESP:Exit()
end
...
getgenv().GandruESP = {
	DeveloperSettings = {
		Path = "GandruHub/GandruESP/Configuration.cfg",
...
local Environment = getgenv().GandruESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("GandruESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "GandruESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("GandruESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "GandruESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "GandruESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "GandruESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().GandruESP[Index] = nil
		end

		LoadESP = nil; Recursive = nil; Loaded = false

		getgenv().GandruESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "GandruESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "GandruESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "GandruESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().GandruESP.DeveloperSettings = DeveloperSettings
	getgenv().GandruESP.Settings = Settings
	getgenv().GandruESP.Properties = Properties

	Environment = getgenv().GandruESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "GandruESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "GandruESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
